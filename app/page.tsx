...
export default function HomePage() {
  ...
  return (
    <div
      ...
      onDrop={handleDrop}
      style={{
        outline: dragging ? '3px dashed var(--brand)' : 'none',
        outlineOffset: -3,
    ...
    >
      ...
      <section style={{ textAlign: 'center', padding: '72px 16px 48px' }}>
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 800, letterSpacing: '-0.04em', lineHeight: 1.15, margin: '0 0 16px', color: 'var(--ink)' }}>
          100+ Tools. Free and Private Forever.
        </h1>
        <p style={{ fontSize: 'clamp(1rem, 2vw, 1.15rem)', color: 'var(--ink-2)', margin: '0 auto 36px', maxWidth: 620, lineHeight: 1.6 }}>
        ...
        <div style={{ position: 'relative', maxWidth: 580, margin: '0 auto 24px' }}>
          ...
          {results.length > 0 && (
            <div className="c-card animate-slide-down" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, marginTop: 6, overflow: 'hidden', textAlign: 'left' }}>
              {results.map(r => (
                <Link key={r.tool.slug} href={`/tools/${r.tool.slug}`} onClick={() => { setQuery(''); setResults([]); }}
                  style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', textDecoration: 'none', borderBottom: '1px solid var(--border)', transition: 'background var(--transition-fast)' }}
                  onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-2)'}
                  onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                >
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 14, color: 'var(--ink)' }}>{r.tool.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--ink-2)' }}>{r.tool.description}</div>
                  </div>
        ...
        {/* Quick action pills — c-btn c-btn--secondary c-btn--sm */}
        ...
        {dragging && (
          <p style={{ fontSize: 14, color: 'var(--brand)', marginTop: 16, fontWeight: 600 }}>
      ...
      <section style={{ maxWidth: 1360, margin: '0 auto', padding: '0 16px 80px' }}>
        <div style={{ marginBottom: 28 }}>
          <h2 style={{ fontSize: 24, fontWeight: 800, margin: '0 0 6px', letterSpacing: '-0.03em', color: 'var(--ink)' }}>
            Categories
          </h2>
      ...
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '0 16px 80px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h2 style={{ fontSize: 22, fontWeight: 700, margin: 0, letterSpacing: '-0.02em', color: 'var(--ink)' }}>Popular Tools</h2>
          <Link href="/tools" style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 13, color: 'var(--brand)', textDecoration: 'none', fontWeight: 500 }}>
      ...
      {/* ── Bottom CTA ─────────────────────────────────────────────── */}
      <section style={{ background: 'var(--bg-1)', borderTop: '1px solid var(--border)', padding: '64px 16px', textAlign: 'center' }}>
        <div style={{ maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 40, marginBottom: 16 }}>🔒</div>
          <h2 style={{ fontSize: 28, fontWeight: 700, marginBottom: 12, letterSpacing: '-0.02em', color: 'var(--ink)' }}>Your data never leaves your device</h2>
          <p style={{ color: 'var(--ink-2)', fontSize: 16, lineHeight: 1.6, margin: '0 0 24px' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
            {['No uploads', 'No sign-up', 'No tracking', 'No cloud', 'Open instantly'].map(t => (
              <span key={t} className="c-badge c-badge--pos" style={{ padding: '4px 12px', fontSize: 13 }}>✓ {t}</span>
            ))}
          </div>
        </div>
      </section>
