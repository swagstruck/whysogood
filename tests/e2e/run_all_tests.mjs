...
async function main() {
  ...
  console.log('🔍 [STEP 1/6] Verifying Strict TypeScript (npx tsc --noEmit)...');
  let tscPassed = false;
  let tscError = '';
  try {
  ...
  console.log('🧪 [STEP 2/6] Running Simple Mode Tier 1: Feature Coverage Suite...');
  const t1 = await runTier1Tests();

  // Step 3: Run Tier 2: Boundary & Corner Cases
  console.log('\n🧪 [STEP 3/6] Running Simple Mode Tier 2: Boundary & Corner Cases Suite...');
  const t2 = await runTier2Tests();

  // Step 4: Run Tier 3: Cross-Feature Combinations
  console.log('\n🧪 [STEP 4/6] Running Simple Mode Tier 3: Cross-Feature Combinations Suite...');
  const t3 = await runTier3Tests();

  // Step 5: Run Tier 4: Real-World Application Scenarios
  console.log('\n🧪 [STEP 5/6] Running Simple Mode Tier 4: Real-World Application Scenarios Suite...');
  const t4 = await runTier4Tests();

  // Step 6: Run File Compression Upgrade Suite (Tiers 1-4)
  console.log('\n🧪 [STEP 6/6] Running File Compression Upgrade E2E Suite (Tiers 1-4)...');
  const tCompress = await runCompressionTests();

  // Aggregate Metrics
  const totalBehavioralTests = t1.total + t2.total + t3.total + t4.total + tCompress.total;
  const totalBehavioralPassed = t1.passed + t2.passed + t3.passed + t4.passed + tCompress.passed;
  const totalBehavioralFailed = t1.failed + t2.failed + t3.failed + t4.failed + tCompress.failed;
